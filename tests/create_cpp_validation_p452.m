...
for iname = 1 : length(filenames)
    ...
    fid = fopen(['test' num2str(count) '.cpp'], 'w');
...
fprintf(fid,'#include <gtest/gtest.h>  // googletest header file\n');

fprintf(fid,'#include <filesystem>\n');

fprintf(fid,'#include "../include/P452.hpp"\n');

fprintf(fid,'using std::filesystem::current_path;\n');

fprintf(fid,'inline std::string\n');
fprintf(fid,'resolvePath(const std::string& relPath)\n');
fprintf(fid,'{\n');
fprintf(fid,'  auto baseDir = std::filesystem::current_path();\n');
fprintf(fid,'  while (baseDir.has_relative_path()) {\n');
fprintf(fid,'    auto test = std::filesystem::path(relPath);\n');
fprintf(fid,'   auto combinePath = baseDir / test;\n');
fprintf(fid,'    if (std::filesystem::exists(combinePath)) {\n');
fprintf(fid,'      return combinePath.string();\n');
fprintf(fid,'    }\n');
fprintf(fid,'    baseDir = baseDir.parent_path();\n');
fprintf(fid,'  }\n');
fprintf(fid,'  throw std::runtime_error("File not found!" + relPath);\n');
fprintf(fid,'}\n');

fprintf(fid,'TEST(P452, Test%d) {\n', count);
count = count + 1;
fprintf(fid,'std::string path = resolvePath("src/maps/");\n');
...
fprintf(fid, 'for (int ii = 0; ii < %d; ii++) {\n', nrows);
 
 
fprintf(fid, 'double result = tl_p452(maps,ff[ii], pp[ii], rDisti, rHeighti, gHeighti, iZonei, htg, hrg, phit_e, phit_n, phir_e, phir_n,  Gt, Gr, pol, dct, dcr, press, temp);\n');
fprintf(fid, 'EXPECT_NEAR(expectedResult[ii], result, %f);\n',tol);
fprintf(fid, '  }\n');
fprintf(fid, '}\n');

% fprintf(fid, 'int main(int argc, char** argv) {\n');
% fprintf(fid, '  ::testing::InitGoogleTest(&argc, argv);\n');
% fprintf(fid, '  return RUN_ALL_TESTS();\n');
% fprintf(fid, '}\n');
